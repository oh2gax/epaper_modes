...
- **Added a screenshot to the README** (real hardware photo, right after the Overview section) showing the display with two flagged cards - one with the Russian star icon, one with the military airscrew icon - scaled to half its native size for the README via an HTML `<img width>` tag, since plain markdown image syntax has no size control.
